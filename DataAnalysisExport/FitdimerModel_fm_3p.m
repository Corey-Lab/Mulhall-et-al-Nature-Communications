function [fitout] = FitdimerModel_fm_3p(fbN,koffN,C,fc,konN,model,rdimer,Fdimer,W,method,dna)
%Fit dimer model using fminsearch using for fit 3 fit parameters: kon, koff
%and fb, Input paramters should be C and fc.
...
switch(method)
    ...
    case(2)
        Xsq=@(x)sum((DimerForceNumericalSolution_Median(rdimer,x(1),x(2),C,fc,x(3),model,dna)-Fdimer).^2.*W);
end
...
disp(['kon(1/M/sec) : ',num2str(fitout.kon),'+/- ',num2str(fitout.dkon),' Q: ', num2str(fitout.Q(3))]);
