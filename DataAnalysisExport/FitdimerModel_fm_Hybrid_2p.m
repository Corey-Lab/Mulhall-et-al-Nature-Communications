function [fitout] = FitdimerModel_fm_Hybrid_2p(fb_1,koff_1,kon_1,C,fc,fbN,koffN,konN,model,rdimer,Fdimer,W,method,dna)
%Fit dimer model using fminsearch using for fit paramters kon_2,koff_2, and
%same fb for both bonds.
...
switch(method)
    ...
    case(2)
        Xsq=@(x)sum((DimerForceNumericalSolution_Hybrid_Median(rdimer,fb_1,fbN,koff_1,x(1),C,fc,kon_1,x(2),model,dna)-Fdimer).^2.*W);
end
...
disp(['kon : ',num2str(fitout.kon),'+/- ',num2str(fitout.dkon),' Q: ', num2str(fitout.Q(2))]);
